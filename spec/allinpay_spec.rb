...
EOF
      ...
      it do
        ...
        expect(Lakala.config.app_id).to eq '123'
        expect(Lakala.config.serial_no).to eq '456'
        expect(Lakala.config.private_key).to_not be_nil
        expect(Lakala.config.public_key).to_not be_nil
        expect(Lakala.config.sandbox_mode).to be_truthy
      end
    end
